...
import qlora_llm.models.model as llama
from qlora_llm.models.lora import LoRALinear, LoRALinear4bit, Linear4bit
...
def _get_lora_kwargs(args: LoraModelArgs) -> Dict:
    return {
        'r': args.lora_r,
...
def _get_quant_kwargs(args: LoraModelArgs) -> Dict:
    return {
        'compress_statistics': args.quant_4bit_double,
        'quant_type': args.quant_4bit_type,
        'compute_dtype': args.quant_compute_dtype,
    }


def _get_lora_linear_layer(args: LoraModelArgs) -> Union[LoRALinear, LoRALinear4bit]:
    layer_cls = None
    kwargs = _get_lora_kwargs(args)
    if args.quant_lora_4bit:
        kwargs.update(_get_quant_kwargs(args))
...
def _get_linear_layer(args: LoraModelArgs) -> Union[nn.Linear, Linear4bit]:
    layer_cls = None
    kwargs = {}
    if args.quant_4bit:
        kwargs.update(_get_quant_kwargs(args))
...
def get_linear_layer(args: LoraModelArgs, use_lora: bool = False) -> Union[nn.Linear, Linear4bit, LoRALinear, LoRALinear4bit]:
    if use_lora:
        return _get_lora_linear_layer(args)
    else:
        return _get_linear_layer(args)
